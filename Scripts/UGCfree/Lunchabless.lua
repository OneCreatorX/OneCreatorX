...
local function Rejoin()
	...
	if #Players:GetPlayers() <= 1 then
		...
		task.wait()
		TeleportService:Teleport(placeId, Players.LocalPlayer)  -- Usar placeId (minúscula)
	else
		TeleportService:TeleportToPlaceInstance(placeId, JobId, Players.LocalPlayer)  -- Usar placeId (minúscula)
	end
end
...
local function iterateMeshParts()
    while true do
        ...
        if #meshParts == 0 then
            ...
            local noCoinsLabel = screenGui:FindFirstChild("NoCoinsTextLabel")
