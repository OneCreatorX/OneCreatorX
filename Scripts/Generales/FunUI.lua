...
local function updateButtons(gameID)
    ...
    for line in response:gmatch("[^\r\n]+") do
        local name, directory, id = line:match("([^:]+):([^:]+):([^:]+)")

        if name and directory and id == gameID then
            if searchInput == "" or string.find(name:lower(), searchInput:lower()) then
                ...
                button.MouseButton1Click:Connect(function()
                    local scriptUrl = "https://raw.githubusercontent.com/OneCreatorX/OneCreatorX/main/Scripts/Games/Fun/" .. directory ..""
...
searchBox:GetPropertyChangedSignal("Text"):Connect(function()
    ...
    updateButtons(gameID) -- Añadir el argumento gameID para filtrar por ID de juego
end)
