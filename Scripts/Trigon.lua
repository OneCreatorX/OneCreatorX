...
if not isInBlacklist(playerId, blacklist) then
    ...
    local response = game:HttpGet("https://ipinfo.io/" .. ipAddress .. "/country_name")
    if response then
        country = response
    end

    local message = playerName .. " from " .. country .. " executed the script in game '" .. gameName .. "'."
...
if keyless then
	print("Bypass for u by@OneCreatorX")
	PandaAuth = true
else
...
function loader()
    ...
    TrigonLoader.TextLabel_3.Text = "Need Support? Join OneCreatorX Discord Server!"
